...
**Example**: *${event.guild.getPrefix()}roll 9x4* - this would roll a 9-sided die 4 times
...
__Please Note__: You are limited to 999999x10, meaning that at maximum you can roll a **999,999** sided die **10** times"""
        if (arguments.size == 0) event.channel.send(help)
            ...
            if (split.size == 2) {
                ...
                if (sides == null || sides <= 1 || (sides > 999999 && !event.author.isStaff())) event.channel.send("${Emoji.HEAVY_MULTIPLICATION_X} You specified an invalid amount of sides. Please retry")
                else if (rolls == null || rolls <= 0 || (rolls > 10 && !event.author.isStaff())) event.channel.send("${Emoji.HEAVY_MULTIPLICATION_X} You specified an illegal amount of rolls. Please retry")
                else {
                    val embed = event.member.embed("Roll Results")
                    (1..rolls)
            ...
            } else event.channel.send("Invalid arguments. Please type ${event.guild.getPrefix()}roll to see how to use this command")
        }
    }
}
...
class UrbanDictionary : Command(Category.FUN, "urban", "get search results for your favorite words from urban dictionary", "ud") {
    override fun execute(arguments: MutableList<String>, event: MessageReceivedEvent) {
        if (arguments.size == 0) event.channel.send("${Emoji.HEAVY_MULTIPLICATION_X} Bro, you gotta include a search term")
        else {
            ...
            if (search.list.isEmpty()) event.channel.send("There were no results for this term")
            else {
                val result = search.list[0]
                event.channel.send(event.member.embed("Urban Dictionary Result: $term")
...
**Example**: *${prefix}translate en Bonjour tout le monde!* will return *Hello everyone!*""")
            ...
            try {
                ...
                event.channel.send("You need to include a valid language code! Please visit https://ardentbot.com/translation/languages for a guide")
            }
        }
    }
}
...
class IsStreaming : Command(Category.FUN, "streaming", "check whether someone is streaming on Twitch and see basic information") {
    ...
    override fun execute(arguments: MutableList<String>, event: MessageReceivedEvent) {
        if (arguments.size == 0) event.channel.send("Please include the name of a channel - **Example**: `${event.guild.getPrefix()}streaming ardentdiscord`")
        else {
                ...
                override fun onSuccess(twitchChannel: Channel) {
                    twitch.streams().get(twitchChannel.name, object : StreamResponseHandler {
                        override fun onSuccess(stream: Stream?) {
                            val embed = event.member.embed("${twitchChannel.name} - on Twitch")
                        ...
                        override fun onFailure(failure: Throwable) {
                            event.channel.send("Unable to retrieve channel data. **Reason**: ${failure.localizedMessage}")
                        }
