    ...
    override fun executeBase(arguments: MutableList<String>, event: MessageReceivedEvent) {
        if (arguments.size == 0 || !arguments[0].equals("set", true)) {
            event.channel.send(event.member.embed("Ardent | Server Language".tr(event), Color.BLUE)
                    .appendDescription("Your server data is **{0}** - You can change it by using {1}lang set **data** - Language list: {2}".tr(event).trReplace(event.guild, event.guild.getLanguage().readable, event.guild.getPrefix(),
                            Language.values().map {
            ...
            if (event.member.hasOverride(event.textChannel)) {
                ...
                if (lang == null) event.channel.send("You specified an invalid data! Remember: You must add accents if your data requires that. Type **{0}lang** to see a data list".tr(event).trReplace(event.guild, event.guild.getPrefix()))
                else {
                    val guildData = event.guild.getData()
                    guildData.languageData = lang
                    guildData.update()
...
As follows are the data codes of some languages, but if you don't see the code for the data you want, go to {1} to view a full list.
