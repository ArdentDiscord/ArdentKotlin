...
class VoiceUtils {
        ...
        waiter.executor.schedule({
            if (member.voiceState.channel != null && member.voiceState.channel != null && member.voiceState.channel == e.channelLeft && e.channelLeft.members.size == 1) {
                if (e.guild.getData().musicSettings.stayInChannel == false) {
                    ...
                    e.guild.getGuildAudioPlayer(null).scheduler.channel?.send("Need me to stay in this voice channel? You can enable it at {0}".tr(e.guild, "<https://ardentbot.com/manage/${e.guild.id}>"))
                    managers.remove(e.guild.idLong)
                }
                else e.guild.getGuildAudioPlayer(null).player.isPaused = true
            }
        }, 15, TimeUnit.SECONDS)
    }
    ...
    fun onVoiceConnect(e: GuildVoiceJoinEvent) {
        if (e.member != e.guild.selfMember) {
            if (e.channelJoined.members.size == 2 && e.guild.getData().musicSettings.stayInChannel == true) e.guild.getGuildAudioPlayer(null).player.isPaused = false
        }
    }
}